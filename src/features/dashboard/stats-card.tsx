...
export function StatsCard({
    ...
    return (
        <Card className={cn("p-4 flex flex-col h-full relative group rounded-xl", className)}>
            {chart && (
                <div className="absolute top-3 right-3 z-10 opacity-0 group-hover:opacity-100 transition-opacity">
                    <Dialog>
                        <DialogTrigger asChild>
                            <Button variant="outline" size="icon" className="h-7 w-7 bg-background/80 backdrop-blur shadow-sm rounded-xl">
                                <Maximize2 className="h-3.5 w-3.5" />
                                <span className="sr-only">Full screen</span>
                            </Button>
                        </DialogTrigger>
                        <DialogContent className="max-w-[95vw] w-full sm:max-w-[900px] h-auto max-h-[90vh] rounded-xl">
                            <DialogHeader>
            ...
            <CardHeader className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-0 p-0 pb-2">
                ...
                <div className="flex flex-col gap-0.5 sm:border-l sm:px-4 text-left mr-8 sm:mr-0">
                    <span className="text-[10px] text-muted-foreground uppercase tracking-widest opacity-70">Total</span>
